$(function () {
    ...
    // 右边商品加减组件
    Vue.component('car-operate',{
        template: `
            <span>
			    <i class="iconfont icon-jian-copy" v-show="foods.count" @click="minus()"></i>
			    <span class="num" v-show="foods.count"> {{ foods.count }} </span>
                <i class="iconfont icon-jjia-" @click="add()"></i>
		    </span>
        `,
        props: ['foods'],
        methods: {
        ...
        <div>
            ...
            <div class="shopping">
                ...
                <div class="iscroll" id="shopscroll">
                    <ul>
                        <li v-for="(item,index) in selected" :key="index + 1">
                            <div class="left"> {{ item.name }} </div>
                            <div class="right">
                                <div class="money">&yen; {{ item.count*item.price }} </div>
                                <car-operate :foods="item"></car-operate>
                            </div>
                        </li>
                    </ul>
                </div>
            ...
            <!--结算栏-->
            <div class="pay" :class="{hot:numbers}">
                <!--购物车-->
                <div class="cart">
                ...
                <div class="left">
                    <p>
                        <span class="discount"  v-show="!discount">未选购商品</span>
                        <span class="discount"  v-show="discount">&yen; {{ discount }} </span>
                        <del class="price"  v-show="discount">&yen; {{ price }} </del>
                    </p>
                    <span>另需配送费11元</span>
                </div>
                <div class="right" :class="{hot:discount>=20}">
                    <span v-show="discount <= 0">20元起配送</span>
                    <span v-show="discount > 0 && discount < 20">还差 {{ 20 - discount }} 元</span>
                    <span v-show="discount >= 20">立即支付</span>
                </div>
            </div>
        </div>
        `,
            ...
            goods: [
                {
                    ...
                    "foods": [
                        {
                            "id": 1,
                            "numbers": 0,"priceTotal": 0,"oldPriceTotal": 0,
                            "name": "皮蛋瘦肉粥",
                            "price": 10,
                            "oldPrice": "",
                            "description": "咸粥",
